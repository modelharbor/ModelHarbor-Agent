...
export async function getModelHarborModels(apiKey?: string): Promise<Record<string, ModelInfo>> {
	try {
		...
		if (!response.ok) {
			throw new Error(`ModelHarbor API responded with status ${response.status}: ${response.statusText}`)
		}
		...
		console.error("❌ Failed to fetch ModelHarbor models:", error.message ? error.message : error)

		if (error.name === "AbortError") {
			console.error("ModelHarbor API request timed out after 5 seconds")
		} else if (error.message && error.message.includes("Failed to fetch")) {
			console.error("Network error when fetching ModelHarbor models. Check internet connection.")
		} else {
			console.error(`Failed to fetch ModelHarbor models: ${error.message || "An unknown error occurred."}`)
		}

		// Return fallback models instead of throwing to allow graceful degradation
