	...
	implements SingleCompletionHandler
{
	...
	override async *createMessage(
		...
		// v3.38.0 API compatibility: Add mergeToolResultText to prevent reasoning models from dropping thinking content
		...
		try {
			...
			// v3.38.0 API compatibility: Track active tool call IDs for tool_call_end events
			...
			for await (const chunk of stream) {
				...
				if (delta?.tool_calls) {
					for (const toolCall of delta.tool_calls) {
						// v3.38.0 API compatibility: Track tool call IDs
						if (toolCall.id) {
				...
				// v3.38.0 API compatibility: Emit tool_call_end events when finish_reason is "tool_calls"
