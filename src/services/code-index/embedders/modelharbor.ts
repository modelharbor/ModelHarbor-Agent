...
export class ModelHarborEmbedder implements IEmbedder {
	 ...
	 * @param options API handler options containing the ModelHarbor API key
	 */
	constructor(options: ApiHandlerOptions & { modelHarborApiKey?: string; modelHarborEmbeddingModelId?: string }) {
		const apiKey = options.modelHarborApiKey
		if (!apiKey) {
		...
		this.defaultModelId = options.modelHarborEmbeddingModelId || "baai/bge-m3"
	}
	...
	async validateConfiguration(): Promise<{ valid: boolean; error?: string }> {
		return withValidationErrorHandling(async () => {
			...
			if (actualDimension) {
				console.log(`[ModelHarborEmbedder] Model ${this.defaultModelId} has dimension: ${actualDimension}`)
			}
