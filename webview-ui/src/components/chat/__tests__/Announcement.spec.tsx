	...
	useAppTranslation: () => ({
		t: (key: string, options?: { version: string }) => {
			...
			if (key === "chat:announcement.description") {
				return `ModelHarbor Agent ${options?.version} brings powerful new features and improvements based on your feedback.`
			}
...
describe("Announcement", () => {
	...
	it("renders the announcement with the version number from package.json", () => {
		...
		expect(
			screen.getByText(
				`ModelHarbor Agent ${expectedVersion} brings powerful new features and improvements based on your feedback.`,
			),
		).toBeInTheDocument()
	})
})
