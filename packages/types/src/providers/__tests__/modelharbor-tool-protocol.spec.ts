...
describe("ModelHarbor Tool Protocol Selection (Types Package)", () => {
	...
	it("should set defaultToolProtocol to xml for non-haiku-4.5 models from real API", async () => {
		const models = await getModelHarborModels()

		// Check sonnet models use xml protocol
		const sonnetModel = models["anthropic/claude-sonnet-4.5-code"]
		if (sonnetModel) {
			expect(sonnetModel.defaultToolProtocol).toBe("xml")
		}

		const opusModel = models["anthropic/claude-opus-4.5-code"]
		if (opusModel) {
			expect(opusModel.defaultToolProtocol).toBe("xml")
		}
