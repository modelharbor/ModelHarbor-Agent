...
function transformModelInfo(apiModel: ModelInfoResponse): ModelInfo {
	...
	// Priority: supports_native_tools > supports_function_calling > false
	const supportsNativeTools =
		model_info.supports_native_tools !== undefined
			? model_info.supports_native_tools
			: (model_info.supports_function_calling || false)

	// Use native tool call protocol as default for all models
	const defaultToolProtocol = "native" as const
...
export const fallbackModelHarborModels = {
	...
	"glm-4.7": {
		...
		inputPrice: 1.0,
		outputPrice: 5.0,
		description: "GLM 4.7 model with strong general-purpose capabilities and large context window.",
	},
...
export const modelHarborDefaultModelId: ModelHarborModelId = "glm-4.7"
